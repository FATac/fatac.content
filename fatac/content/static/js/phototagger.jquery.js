	...
	PhotoTagger.prototype = {
		...
		loadTagRecords: function( onSuccess ){
			var self = this;
                        ...
                        // Load the tag records. NEW METHOD
                        $.getJSON(this.settings.loadURL, function(response) {
                            // Pass off to handler (if it exists).
                            if (onSuccess){
                                onSuccess(
                                    // Clean the response before handing
                                    // it off to the handler.
                                    self.settings.cleanAJAXResponse("delete", response)
                                    );
                            }
                        });

			// OLD METHOD!!! Load the tag records.
                        /*
			$.ajax({
				method: "get",
				...
				dataType: "json",
				cache: false,
                                //complete: function(data){ alert(data.id); },
				success: function( response ){
					// Pass off to handler (if it exists).
					if (onSuccess){
						onSuccess(
		...
		saveTagRecord: function( id, x, y, width, height, message, photoID, onSuccess ){
			var self = this;
                        ...
                        tagInfo = [id, x, y, width, height, message, photoID];
                        $.ajax({
                               url: this.settings.saveURL + "?tag=" + tagInfo, 
                               type: "post",
                               success: function( response ){
                                   // Pass off to handler (if it exists).
                                   if (onSuccess){
